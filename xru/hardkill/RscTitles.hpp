class xru_hardkill_hud {
    idd = -1;
    onLoad = "with uiNameSpace do {xru_hardkill_hud = _this select 0 };";
    movingEnable = 0;
    enableSimulation = 1;
    duration = 60;
    fadeIn = "false";
    ...
    class controls {
        class ControlGroup: RscControlsGroup {
            ...
            y = "2.5 * ((((safezoneW / safezoneH) min 1.2) / 1.2) / 25) + (profilenamespace getVariable ['IGUI_GRID_WEAPON_Y', (safezoneY + 0.5 * ((((safezoneW / safezoneH) min 1.2) / 1.2) / 25))]) + ((((safezoneW / safezoneH) min 1.2) / 1.2) / 25)";
