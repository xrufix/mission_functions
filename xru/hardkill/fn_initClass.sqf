...
if (count (GVAR(classHash) select {_x select 0 == typeOf _vehicle}) != 0) exitWith {
...
[_classname, 1, ["ACE_SelfActions"], GVAR(interaction_root), true] call ace_interact_menu_fnc_addActionToClass;
[_classname, 1, ["ACE_SelfActions", QGVAR(interaction_root)], GVAR(interaction_enable), true] call ace_interact_menu_fnc_addActionToClass;
[_classname, 1, ["ACE_SelfActions", QGVAR(interaction_root)], GVAR(interaction_disable), true] call ace_interact_menu_fnc_addActionToClass;
[_classname, 1, ["ACE_SelfActions", QGVAR(interaction_root)], GVAR(interaction_checkAmmo), true] call ace_interact_menu_fnc_addActionToClass;

if (isServer) then {
    [_classname, "init", {
        params ["_vehicle"];
        private _args = GVAR(classHash) select {_x select 0 == typeOf _vehicle} select 0;
